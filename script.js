(() => {
  ...
  function updateFlow() {
    ...
    const travel = Math.max(1, rect.height - viewport + stickyOffset);

    // 0 quando la sezione entra nella fase sticky, 1 quando termina.
    const progress = clamp((stickyOffset - rect.top) / travel);
    ...
    flowSection.classList.toggle('flow-complete', progress >= 0.995);

    let currentIndex = 0;
    flowItems.forEach((item, index) => {
      const threshold = flowItems.length <= 1 ? 0 : index / (flowItems.length - 1);
      const reached = progress + 0.018 >= threshold;
    ...
    if (flowItems.length && progress > 0.01 && progress < 0.999) {
