...
export function deliveryEmailHtml({ customerName, orderId, downloadUrl, expiresDays, maxDownloads }) {
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f4f7f9;padding:32px 12px"><tr><td align="center">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:620px;background:#ffffff;border:1px solid #dce6eb;border-radius:22px;overflow:hidden">
      <tr><td style="padding:34px 36px 16px"><div style="font-size:13px;letter-spacing:.14em;color:#0aa8b8;font-weight:700">HOST OS 1.2</div><h1 style="font-size:28px;line-height:1.15;margin:10px 0 12px">Il tuo HOST OS è pronto</h1><p style="font-size:16px;line-height:1.6;color:#536a76;margin:0">Ciao ${esc(customerName)}, il pagamento dell’ordine <strong>${esc(orderId)}</strong> è stato verificato.</p></td></tr>
      <tr><td style="padding:16px 36px"><a href="${esc(downloadUrl)}" style="display:inline-block;background:#24d9d4;color:#08232c;text-decoration:none;font-weight:800;padding:14px 22px;border-radius:12px">Scarica HOST OS 1.2</a></td></tr>
      <tr><td style="padding:8px 36px 34px"><p style="font-size:14px;line-height:1.65;color:#536a76">Il link resta valido per ${esc(expiresDays)} giorni e consente fino a ${esc(maxDownloads)} download. Dopo l’estrazione del pacchetto, inizia da <strong>00_APRI_HOST_OS.html</strong> e consulta Quick Start.</p>${support ? `<p style="font-size:14px;color:#536a76">Supporto: <a href="mailto:${esc(support)}">${esc(support)}</a></p>` : ''}<p style="font-size:12px;color:#8a9aa3">Prodotto digitale HOST OS · Ordine ${esc(orderId)}</p></td></tr>
    </table>
