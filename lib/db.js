...
export async function ensureSchema() {
  ...
  schemaReady = (async () => {
    const db = getPool();
    await db.query(`
      CREATE TABLE IF NOT EXISTS host_orders (
        id TEXT PRIMARY KEY,
      ...
      CREATE INDEX IF NOT EXISTS host_orders_email_idx ON host_orders(email);
      CREATE INDEX IF NOT EXISTS host_orders_status_idx ON host_orders(order_status);
      CREATE INDEX IF NOT EXISTS host_orders_created_idx ON host_orders(created_at DESC);
...
export async function getOrder(id) {
  await ensureSchema();
  const q = await getPool().query('SELECT * FROM host_orders WHERE id=$1 LIMIT 1', [id]);
  return q.rows[0] || null;
}

export async function getOrderByCustomerToken(id, tokenHash) {
  await ensureSchema();
  const q = await getPool().query('SELECT * FROM host_orders WHERE id=$1 AND customer_token_hash=$2 LIMIT 1', [id, tokenHash]);
  return q.rows[0] || null;
}

export async function getOrderByPayPalId(paypalOrderId) {
  await ensureSchema();
  const q = await getPool().query('SELECT * FROM host_orders WHERE paypal_order_id=$1 LIMIT 1', [paypalOrderId]);
  return q.rows[0] || null;
}

export async function setPayPalOrder(id, paypalOrderId) {
  await ensureSchema();
  const q = await getPool().query(`UPDATE host_orders SET paypal_order_id=$2, updated_at=NOW() WHERE id=$1 RETURNING *`, [id, paypalOrderId]);
  return q.rows[0] || null;
}
...
export async function claimDelivery(id, force = false) {
  await ensureSchema();
  const q = await getPool().query(`
    UPDATE host_orders SET delivery_status='PROCESSING', delivery_error=NULL, updated_at=NOW()
...
export async function markDeliveryError(id, message) {
  await ensureSchema();
  const q = await getPool().query(`
    UPDATE host_orders SET delivery_status='EMAIL_FAILED', delivery_error=$2, updated_at=NOW()
    WHERE id=$1 RETURNING *
...
export async function getOrderByDownloadToken(tokenHash) {
  await ensureSchema();
  const q = await getPool().query(`SELECT * FROM host_orders WHERE download_token_hash=$1 LIMIT 1`, [tokenHash]);
  return q.rows[0] || null;
}
...
export async function consumeDownload(id) {
  await ensureSchema();
  const q = await getPool().query(`
    UPDATE host_orders SET download_count=download_count+1, updated_at=NOW()
...
export async function recordWebhookEvent(eventId, eventType) {
  await ensureSchema();
  const q = await getPool().query(`
    INSERT INTO host_webhook_events(event_id,event_type) VALUES($1,$2)
...
export async function listOrders({ status = '', method = '', limit = 100 } = {}) {
  ...
  const q = await getPool().query(`SELECT * FROM host_orders ${where} ORDER BY created_at DESC LIMIT $${values.length}`, values);
  return q.rows;
}
...
export async function setOrderState(id, state) {
  ...
  const q = await getPool().query(`
    UPDATE host_orders SET order_status=$2,
